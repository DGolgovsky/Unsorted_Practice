...
crack( std::vector<std::string> const &words
	 , std::string const &loginAttempt )
{
	if (memos.find(loginAttempt) == memos.end()) {
		for (auto const& word : words) {
			if (loginAttempt.compare(0, word.length(), word) == 0) {
				auto s = loginAttempt.substr(word.length());
                if (s.empty())
                    return new std::vector<std::string> {word};

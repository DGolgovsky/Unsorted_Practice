 ...
 * т.е. минимальное количество бит,
 * которыми можно представить данное число
 */
template <class T>
...
/*!
 * Вычисляет pi(b) первых простых чисел.
 * Возвращает ссылку на вектор с простыми
 * (в векторе может оказаться больше простых, чем надо) и в pi - pi(b)
 */
template <class T, class T2>
const std::vector<T>& get_primes(const T& b, T2& pi)
{
    ...
    if (counted_b >= b) {
        ...
        for (T cur=first; cur<=b; ++++cur) {
            ...
            for (typename std::vector<T>::const_iterator iter = primes.cbegin(), end = primes.cend();
				iter != end; ++iter) {
                const T& div = *iter;
                if (div * div > cur)
                    break;
 ...
 * Результат:
 * 1 - если n точно простое, p - его найденный делитель,
 * 0 - если неизвестно, является ли n простым или нет
 */
template <class T, class T2>
T2 prime_div_trivial(const T& n, T2 m)
{
    ...
    for (typename std::vector<T2>::const_iterator iter=primes.cbegin(), end=primes.cend();
...
/*!
 * Усиленный алгоритм Лукаса-Селфриджа проверки n на простоту.
 * Используется усиленный алгоритм Лукаса с параметрами Селфриджа.
 * Работает только с знаковыми типами.
 * Второй параметр unused не используется, он только дает тип
 */
template <class T, class T2>
